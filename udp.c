...
// void init_or_load_fs(const char *fs_image) {
//     fd = open(fs_image, O_RDWR | O_CREAT, 0666); // Open or create the file system image
//     if (fd < 0) {
//         perror("open");
//         exit(EXIT_FAILURE);
//     }
...
//         unsigned char *empty_buffer = calloc(UFS_BLOCK_SIZE, 1); // Allocate zeroed buffer
...
//         lseek(fd, 0, SEEK_SET);
//         int rc = read(fd, &fs_state.superblock, sizeof(super_t));
//         if (rc != sizeof(super_t)) {
//             perror("read");
//             exit(1);
//         }
